...
void GameLogic :: CameraSystem(Entity& player) {
    ...
    if (Game::camera.x + Game::SCREEN_WIDTH > Map::sizeX * 32 * Game::scaleX) Game::camera.x = Map::sizeX * 32 - Game :: SCREEN_WIDTH;
    if (Game::camera.y + Game::SCREEN_HEIGHT > Map::sizeY * 32 * Game::scaleY) Game::camera.y = Map::sizeY * 32 - Game :: SCREEN_HEIGHT;
}
